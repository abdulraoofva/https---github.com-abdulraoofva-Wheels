...
def detect_drowsiness(request):
    # Define your configuration parameters here or retrieve them from Django settings
    ...
    predictor = dlib.shape_predictor("C:\\Users\\raoof\\OneDrive\\Desktop\\Main\\wheelways\\drowsiness\\shape_predictor_68_face_landmarks.dat")  # Specify path
    ...
    vs = VideoStream(src=0).start()  # Adjust webcam source as needed
    time.sleep(1.0)
            ...
            if ear < 0.25:
                ...
                if COUNTER >= 48:
                    ...
                    if not ALARM_ON:
                        ...
                        path_to_alarm = "C:\\Users\\raoof\\OneDrive\\Desktop\\Main\\wheelways\\wheelways\\alarm.wav"  # Specify path to alarm sound
                        t = Thread(target=sound_alarm, args=(path_to_alarm,))
                        t.deamon = True
                        t.start()
    ...
    # Render the template and pass any context variables (such as alert messages)
    return render(request, 'drowsiness_detection.html', {'alert': "Alert: Drowsiness Detected!"})
